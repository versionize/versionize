...
public class DotnetBumpFileTests : IDisposable
{
    ...
    public void ShouldThrowForInconsistentVersions()
    {
        ...
        // Act/Assert
        var testPlatformAbstractions = new TestPlatformAbstractions();
        CommandLineUI.Platform = testPlatformAbstractions;
        var ex = Should.Throw<CommandLineExitException>(() => DotnetBumpFile.Create(_tempDir));
        testPlatformAbstractions.Messages.Single().ShouldContain("have an inconsistent <Version>");
    }
