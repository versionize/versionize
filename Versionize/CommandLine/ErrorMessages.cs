...
/// Messages are crafted to match existing outputs to avoid breaking tests.
/// </summary>
public static class ErrorMessages
{
    ...
    public static string RepositoryNotGit(string path) => $"Directory {path} or any parent directory do not contain a git working copy";
    public static string RepositoryDirty(string path, string dirtyFiles) => $"Repository {path} is dirty. Please commit your changes:\n{dirtyFiles}";
    ...
    public static string InconsistentProjectVersions(string workingDirectory, string versionElement) =>
        $"Some projects in '{workingDirectory}' have an inconsistent <{versionElement}> value. Align all versions or remove <{versionElement}> from projects that should not be versioned.";
    ...
    public static string VersionUnaffected(string currentVersion) => $"Version was not affected by commits since last release ({currentVersion})";
    public static string CouldNotParseReleaseVersion(string releaseAs) => $"Could not parse the specified release version {releaseAs} as valid version";
    public static string SemanticVersionConflict(string next, string current) =>
        $"Semantic versioning conflict: the next version {next} would be lower than the current version {current}. This can be caused by using a wrong pre-release label or release as version";
    public static string VersionAlreadyExists(string version) => $"Version {version} already exists. Please use a different version.";
    ...
    public static string CommandFailed(string command, string args, string error, int exitCode) => $"Command '{command} {args}' failed with error '{error}' (exit code {exitCode})";
    public static string LibGitNotFound(Exception e) => $"""
        ...
        {e}
        """;
    public static string RemoteUrlInvalidSshPattern(string provider, string pushUrl) => $"Remote url {pushUrl} is not recognized as valid {provider} SSH pattern";
    public static string RemoteUrlInvalidHttpsPattern(string provider, string pushUrl) => $"Remote url {pushUrl} is not recognized as valid {provider} HTTPS pattern";
    public static string RemoteUrlNotRecognized(string provider, string pushUrl) => $"Remote url {pushUrl} is not recognized as {provider} SSH or HTTPS url";
    ...
    public static string ProjectMissingVersionElement(string projectFile, string versionElement) =>
        $"Project {projectFile} does not contain a <{versionElement}> XML Element. Please add one if you want to version this project - for example use <{versionElement}>1.0.0</{versionElement}>";
